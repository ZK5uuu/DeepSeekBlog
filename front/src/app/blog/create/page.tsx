...
import { postApi, musicApi } from '@/app/api/services/blogService';
...
// 修改currentTypeConfig定义，在导入文件上方添加
const currentTypeConfig = (contentType: ContentType) => {
  // return contentTypeConfig[contentType] || contentTypeConfig.book;
  return contentTypeConfig[contentType] || contentTypeConfig.music;
};

// 对于标签选择器部分，将整个匿名函数提取出来作为独立组件
interface TagSelectorProps {
  showTagSelector: boolean;
  tagSelectorRef: React.RefObject<HTMLDivElement>;
  tagOptions: string[];
  selectedTags: string[];
  toggleTag: (tag: string) => void;
}

const TagSelector = ({ showTagSelector, tagSelectorRef, tagOptions, selectedTags, toggleTag }: TagSelectorProps) => {
...
export default function CreateBlogPage() {
  ...
  const handleSave = async () => {
    ...
    if (contentType === 'music') {
      ...
      if (!albumImageUrl && !coverImage) {
        alert('请上传专辑封面或设置博客封面');
        return;
      }
    ...
    try {
      // 准备提交的数据
      const blogData = {
        title: contentType === 'music' ? albumName : contentType === 'movie' ? movieTitle : title,
        content,
      ...
      if (contentType === 'music') {
        ...
        const musicData = {
          ...
          artistName: blogData.artistName,
          albumName: blogData.albumName,
          albumImageUrl: blogData.albumImageUrl,
          contentLink: blogData.contentLink,
          username: blogData.username
        };
        
        result = await musicApi.createMusic(musicData);
        console.log('音乐博客创建成功:', result);
      } else {
        // 创建或更新普通博客
      ...
      alert(`${isEditMode ? '更新' : '保存'}失败: ${error.message || '未知错误'}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  // 获取当前内容类型配置
  const currentTypeConfigValue = currentTypeConfig(contentType);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20">
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-6">
                您的{currentTypeConfigValue.label}已成功{isEditMode ? '更新' : '保存'}，即将跳转到{contentType === 'music' ? '音乐列表' : contentType === 'movie' ? '电影列表' : contentType === 'book' ? '书籍列表' : '博客列表'}...
              </p>
          ...
          <div className="flex justify-between items-center">
            <Link href={contentType === 'book' ? "/book" : contentType === 'movie' ? "/movie" : "/music"}>
              <span className="flex items-center text-gray-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors group">
                ...
                返回{contentType === 'book' ? "书籍" : contentType === 'movie' ? "电影" : "博客"}页面
              </span>
            </Link>
            ...
            >
              <div className={`w-3 h-3 rounded-full mr-2 bg-gradient-to-r ${gradientColors[currentTypeConfigValue.color as keyof typeof gradientColors]} animate-pulse`}></div>
              <span className="text-sm font-medium bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {isEditMode ? '编辑' : '创作'}{currentTypeConfigValue.label}
              </span>
            </motion.div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-5xl">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden border border-gray-100 dark:border-gray-700 max-w-3xl mx-auto">
          {/* 标题区域 */}
          <div className={`bg-gradient-to-r ${gradientColors[currentTypeConfigValue.color as keyof typeof gradientColors]} p-8`}>
            {contentType === 'music' ? (
              ...
              <input
                ref={titleRef}
                type="text"
                placeholder={`请输入${currentTypeConfigValue.label}标题...`}
                value={title}
          ...
          <div className="p-8">
            ...
            <div className="mb-8 p-5 bg-gray-50 dark:bg-gray-900/50 rounded-xl">
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-3 flex items-center">
                {currentTypeConfigValue.icon}
                <span className="ml-2">{currentTypeConfigValue.label}信息</span>
              </h3>
          ...
          <input
            ...
            <div className="mb-8">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                ...
                {contentType === 'music' ? '博客内容' : '内容'}
              </label>
              <div className="relative">
                <textarea
                  ref={contentRef}
                  placeholder={contentType === 'music' ? '写下对这篇博客的感想...' : `开始写${currentTypeConfigValue.label}内容...`}
                  value={content}
            ...
            <div className="mt-10 mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              {contentType === 'music' ? (
                ...
                <>
                  ...
                  <motion.button 
                    className={`flex items-center justify-center bg-gradient-to-r ${gradientColors[currentTypeConfigValue.color as keyof typeof gradientColors]} text-white px-8 py-3 rounded-xl shadow-md hover:shadow-lg disabled:opacity-70 disabled:cursor-not-allowed text-base font-medium`}
                    onClick={handleSave}
                ...
                <>
                  ...
                  >
                    {isSaving ? (
                      ...
                      <>
                        <FaSave className="mr-2" /> 
                        发布{currentTypeConfigValue.label}
                      </>
                    )}
                  </motion.button>
                </>
